...
export default function CardDétailsRemb() {
  ...
  const fetchRembDetails = async () => {
    try {
      const response = await fetch(`${SummaryApi.getRemboursement.url}/${id}`, {
        method: SummaryApi.getRemboursement.method,
        headers: {
      ...
      toast.error("Erreur lors de la récupération des détails de l'rembourssement.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchUserDetails = async () => {
    try {
      const response = await fetch(`${SummaryApi.getUserDetailsByRembId.url}/${id}`, {
        ...
        <div className="hover:-mt-4 relative flex flex-col min-w-0 break-words  bg-orange-dys w-full mb-6 shadow-lg rounded-lg bg-gradient-to-r from-orange-400 to-orange-600 ease-linear transition-all duration-150">
          ...
          <div className="p-8">
            <h4 className="text-xl font-bold text-white text-center mb-4">
              Sujet de Réclamation : {rembourssement?.SujetReclamation}
            </h4>
            <div className="space-y-4">
              <div className="flex items-center text-white">
                <FaUser className="mr-2" />
                <span>Montant : {rembourssement?.Montant}</span>
              </div>
              <div className="flex items-center text-white">
                <FaCalendarAlt className="mr-2" />
                <span>Date prévu de rembourssement : {new Date(rembourssement?.DatePrevu).toLocaleDateString()}</span>
              </div>
              <div className="flex items-center text-white">
